...
def create_xgb_confusion_matrix_plot(metrics_snapshot: Dict[str, Any]) -> Dict[str, Any]:
...
def create_xgb_classification_report_table(metrics_snapshot: Dict[str, Any]) -> Dict[str, Any]:
    _, _, _, _, class_df, _, _, _ = _get_common_metrics(metrics_snapshot)
    return {"type": "dataframe", "content": class_df}


def create_xgb_performance_plot(metrics_snapshot: Dict[str, Any]) -> Dict[str, Any]:
    ...
    ax.set_ylim(0, 1.05)
    ax.set_ylabel('Score')
    ax.set_title('XGBoost Classifier Performance Metrics')
...
def create_xgb_feature_importance_plot(metrics_snapshot: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if (feature_importances and features and
        ...
        ax.set_title('XGBoost Classifier - Feature Importance Ranking')
...
def model_report():
    ...
    if not found:
        st.error("No XGBoost Classifier model results found. Please train a model first.")
        return

    st.markdown("---")
    st.markdown("## XGBoost Classifier Model Analysis")

    try:
        summary_asset = create_xgb_summary_text(model_results)
        table_asset = create_xgb_classification_report_table(model_results)
        metrics_plot_asset = create_xgb_performance_plot(model_results)
        conf_matrix_asset = create_xgb_confusion_matrix_plot(model_results)
        feature_importance_asset = create_xgb_feature_importance_plot(model_results)
        ...
        st.error(f"Error displaying XGBoost Classifier model report: {e}")
